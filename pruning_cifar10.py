...
save_path = 'E:\\pycharm\\filter-pruning-geometric-median-master\\savelog\\resnet56_1\\resnet56_attentiononly_pretrained_p0.9_lr0.01'
data_path = 'E:\data\cifar-10-batches-py'
dataset = 'cifar10'
batch_size = 128
workers = 8
arch = 'resnet56'
...
def main():
    ...
    log = open(os.path.join(save_path, 'log_seed_{}.txt'.format(manualSeed)), 'w')
    ...
    optimizer = torch.optim.SGD(net.parameters(), learning_rate, momentum=momentum,
                                weight_decay=decay, nesterov=True)

    if use_cuda:
        net.cuda()
        criterion.cuda()
    pretrain_path = 'E:/pycharm/filter-pruning-geometric-median-master/pretrained_model/baseline_without_pruning/cifar10_resnet56_baseline/cifar10_resnet56_base2/checkpoint.pth.tar'
    if use_pretrain:
        if os.path.isfile(pretrain_path):
            print_log("=> loading pretrain model '{}'".format(pretrain_path), log)
        # else:
        #     dir = '/data/yahe/cifar10_base/'
        #     # dir = '/data/uts521/yang/progress/cifar10_base/'
        #     whole_path = dir + 'cifar10_' + arch + '_base'
        #     pretrain_path = whole_path + '/checkpoint.pth.tar'
        #     print_log("Pretrain path: {}".format(pretrain_path), log)
    ...
    if resume:
        ...
        print_log("=> do not use any checkpoint for {} model".format(arch), log)
    ...
    print("remaining ratio of pruning : Norm is %f" % rate_norm)
    print("reducing ratio of pruning : Distance is %f" % rate_dist)
    print("total remaining ratio is %f" % (rate_norm - rate_dist))
    ...
    m.init_mask(rate_norm, rate_dist, dist_type)
    #    m.if_zero()
    m.do_mask()
    m.do_similar_mask()
    net = m.model
    ...
    for epoch in range(start_epochs, epochs):
        current_learning_rate = adjust_learning_rate(optimizer, epoch, gammas, schedule)

        need_hour, need_mins, need_secs = convert_secs2time(epoch_time.avg * (epochs - epoch))
        need_time = '[Need: {:02d}:{:02d}:{:02d}]'.format(need_hour, need_mins, need_secs)

        print_log(
            '\n==>>{:s} [Epoch={:03d}/{:03d}] {:s} [learning_rate={:6.4f}]'.format(time_string(), epoch, epochs,
        ...
        val_acc_1, val_los_1 = validate(test_loader, net, criterion, log)
        if epoch % epoch_prune == 0 or epoch == epochs - 1:
            m.model = net
            m.if_zero()
            m.init_mask(rate_norm, rate_dist, dist_type)
            m.do_mask()
        ...
        }, is_best, save_path, 'checkpoint.pth.tar')

        # measure elapsed time
        epoch_time.update(time.time() - start_time)
        start_time = time.time()
        recorder.plot_curve(os.path.join(save_path, 'curve.png'))
...
class Mask:
    ...
    def get_filter_similar(self, weight_torch, compress_rate, distance_rate, length, dist_type="l2"):
        ...
        if len(weight_torch.size()) == 4:
            ...
            if dist_type == "cos":  # for cos similarity
                similar_matrix = 1 - distance.cdist(weight_vec, weight_vec, 'cosine')
            elif dist_type == 'attention':
                weight_matrix = torch.matmul(weight_vec, weight_vec.transpose(0, 1))
                # print('re', weight_matrix)
                weight_attention = torch.nn.functional.softmax(weight_matrix, dim=-1)
                # print('weight_attention', weight_attention)
                weight_diagonal = np.diagonal(weight_attention.cpu().detach().numpy())
            # if dist_type == 'l2' or 'l1':
            #     similar_sum = np.sum(np.abs(similar_matrix), axis=0)
            # else:
            similar_sum = weight_diagonal
            # for distance similar: get the filter index with largest similarity == small distance
            similar_large_index = similar_sum.argsort()[-similar_pruned_num:]
            similar_small_index = similar_sum.argsort()[:  similar_pruned_num]
            #prune large or small
            prune_large = False
            if prune_large:
                #改 不经过norm
                 print('use large_index for prune!!!!!!')
                 similar_index_for_filter = [i for i in similar_large_index]
                # similar_index_for_filter = [filter_large_index[i] for i in similar_large_index]
            else:
                #similar_index_for_filter = [filter_large_index[i] for i in similar_small_index]
                print('use small_index for prune!!!!!!')
                similar_index_for_filter = [i for i in similar_small_index]
