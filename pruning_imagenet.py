...
class Mask:
    ...
    # optimize for fast ccalculation  (使用注意力机制的时候直接把compress_rate设置为0)
    def get_filter_similar(self, weight_torch, compress_rate, distance_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            # 2.for cos similarity
            # similar_matrix = 1 - distance.cdist(weight_vec_after_norm, weight_vec_after_norm, 'cosine')

            # 3.for attention similarity (注意力机制剪枝)
            weight_matrix = torch.matmul(weight_vec_after_norm, weight_vec_after_norm.transpose(0, 1))
            ...
            #for euclidean distance and cos similarity
